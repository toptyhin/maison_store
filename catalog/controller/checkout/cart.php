...
class ControllerCheckoutCart extends Controller {
	public function index() {
		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			$data = array_merge($data, $this->getCartPageData());
	...
	/**
	 * Returns cart page data (products, totals, messages, etc.) for full page or fragment.
	 * Used by index() and content().
	 */
	protected function getCartPageData() {
		...
		if (!$this->cart->hasProducts() && empty($this->session->data['vouchers'])) {
			...
			$data['total_original_value'] = 0;
			$data['total_discount_text'] = '';
			$data['total_discount_percentage'] = 0;
					...
					foreach ($product_options as $po) {
						foreach ($po['product_option_value'] as $pov) {
							if (isset($option[$po['product_option_id']]) && $option[$po['product_option_id']] == $pov['product_option_value_id']) {
								if ($pov['price_prefix'] == '+') $price += (float)$pov['price'];
								elseif ($pov['price_prefix'] == '-') $price -= (float)$pov['price'];
								elseif ($pov['price_prefix'] == '=') $price = (float)$pov['price'];
								break;
							}
						}
	...
	/**
	 * Returns cart content as JSON fragment for AJAX refresh (no header/footer).
	 */
	public function content() {
		$this->load->language('checkout/cart');
		$data = $this->getCartPageData();
		$html = $this->load->view('checkout/cart_content', $data);
	...
	public function edit() {
		...
		if (!empty($this->request->post['quantity'])) {
			...
			$is_xhr = !empty($this->request->server['HTTP_X_REQUESTED_WITH']) && (strtolower($this->request->server['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest');
			if ($is_xhr) {
				$json['success'] = true;
			} else {
