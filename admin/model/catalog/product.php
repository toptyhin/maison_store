				...
				if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
					if (isset($product_option['product_option_value'])) {
						...
						foreach ($product_option['product_option_value'] as $product_option_value) {
							...
							if (!empty($product_option_value['custom_fields']) && is_array($product_option_value['custom_fields'])) {
								foreach ($product_option_value['custom_fields'] as $field) {
									if (!empty($field['key'])) {
										$this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value_field SET product_option_value_id = '" . (int)$product_option_value_id . "', field_key = '" . $this->db->escape($field['key']) . "', field_value = '" . $this->db->escape($field['value']) . "'");
									}
								}
							}
						}
				...
				if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
					if (isset($product_option['product_option_value'])) {
						...
						foreach ($product_option['product_option_value'] as $product_option_value) {
							...
							if ($pov_id > 0) {
								$this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value SET product_option_value_id = '" . $pov_id . "', product_option_id = '" . (int)$product_option_id . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', option_value_id = '" . (int)$product_option_value['option_value_id'] . "', quantity = '" . (int)$product_option_value['quantity'] . "', subtract = '" . (int)$product_option_value['subtract'] . "', price = '" . (float)$product_option_value['price'] . "', price_prefix = '" . $this->db->escape($product_option_value['price_prefix']) . "', points = '" . (int)$product_option_value['points'] . "', points_prefix = '" . $this->db->escape($product_option_value['points_prefix']) . "', weight = '" . (float)$product_option_value['weight'] . "', weight_prefix = '" . $this->db->escape($product_option_value['weight_prefix']) . "'");
