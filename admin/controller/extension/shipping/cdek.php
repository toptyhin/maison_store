...
class ControllerExtensionShippingCdek extends Controller {
	...
	public function index() {
		...
		if (isset($this->request->post['shipping_cdek_cost_pickup'])) {
			$data['shipping_cdek_cost_pickup'] = $this->request->post['shipping_cdek_cost_pickup'];
		} else {
			$data['shipping_cdek_cost_pickup'] = $this->config->get('shipping_cdek_cost_pickup');
		}

		if (isset($this->request->post['shipping_cdek_cost_courier'])) {
			$data['shipping_cdek_cost_courier'] = $this->request->post['shipping_cdek_cost_courier'];
		} else {
			$data['shipping_cdek_cost_courier'] = $this->config->get('shipping_cdek_cost_courier');
		}
	...
	public function install() {
		...
		$this->model_setting_setting->editSetting('shipping_cdek', array(
			'shipping_cdek_cost_pickup'  => 300,
			'shipping_cdek_cost_courier' => 500,
			'shipping_cdek_tax_class_id' => 0,
			'shipping_cdek_geo_zone_id'  => 0,
			'shipping_cdek_status'       => 0,
			'shipping_cdek_sort_order'   => 0
		));
	}
